...
function fim_integrand_num(f, f_c, system::B) where B <: Binary
    ...
    ∂amp₊ = ForwardDiff.gradient(vals -> amp₊(f, Constructor(vals...)), collect(values(system)))
    ∂amp₊[∂amp₊ .=== nothing] .= 0.
    ∂amp₊ = convert(Array{Float64}, ∂amp₊)

    # ∂Ψ = collect(values(gradient(s -> Ψ(f, f_c, s), system)[1]))  # zygote
    ∂Ψ = ForwardDiff.gradient(vals -> Ψ(f, f_c, Constructor(vals...)), collect(values(system)))
    ∂Ψ[∂Ψ .=== nothing] .= 0.
...
"""
function fim(fₗ, fₕ, f_c, system)  # where T <: Binary
